...
def xlate_reg(r):
	regno = int(r[1:])
	return ['', 'rdi','r8', 'r9','r10','r11','r12','r13','r14','r15'][regno]

asm_out = []
asm_out.append('BITS 64')
asm_out.append('push rsp')
asm_out.append('mov rbp, rsp')
asm_out.append('mov rdi, 0x4000000')
asm_out.append('xor rsi, rsi')
asm_out.append('xor r9, r9')
asm_out.append('xor r10, r10')
asm_out.append('xor r11, r11')
asm_out.append('xor r12, r12')
asm_out.append('xor r13, r13')
asm_out.append('xor r14, r14')
asm_out.append('xor r15, r15')

pc = 0
for fmtstr in instrs:
	lmaopc = pc
	asm_out.append(xlate_label(pc) + ':')

	instr = parse_instr(fmtstr)
	code[pc] = instr
	# print(pc, instr)
	pc += len(fmtstr) + 1
	...
	if parts[0].startswith('pc='):
		nextpc_expr = parts[0].split('=')[1]
		if '?' in nextpc_expr:
			tern, const = nextpc_expr.split('+')
			tern_var, tern_val = tern[1:-1].split('?')
			tern_lhs, tern_rhs = map(int, tern_val.split(':'))
			tern_lhs += int(const)
			tern_rhs += int(const)

			x86_reg = xlate_reg(tern_var)
			x86_dst_true, x86_dst_false = xlate_label(tern_lhs), xlate_label(tern_rhs)
			asm = 'test %s, %s\njnz %s' % (x86_reg, x86_reg, x86_dst_true)
			if pc != x86_dst_false:
				asm += '\njmp %s' % (x86_dst_false,)
			# print('cmpbr', tern_var, tern_lhs, tern_rhs)
			asm_out.append(asm)
			continue
		else:
			nextpc = int(nextpc_expr)
	if parts[0] == 'halt':
		asm = 'jmp loc_end;'
		# print('halt')
	elif nextpc != pc:
		assert len(parts) == 1
		asm = 'jmp %s' % (xlate_label(nextpc),)
		# print('wow jmp', nextpc, pc)
	else:
		assert len(parts) == 2
		asm = 'xor eax, eax'
		...
		for subexpr in subexprs:
			try:
				...
				var = subexpr
				if var[0] == '*':
					asm += '\nadd rax, [' + xlate_reg(var[1:]) + ']'
				else:
					asm += '\nadd rax, ' + xlate_reg(var)
		if lhs[0] == '*':
			asm += '\nmov [%s], rax' % (xlate_reg(lhs[1:],))
		else:
			asm += '\nmov %s, rax' % (xlate_reg(lhs,))
	asm_out.append(asm)
	asm_out.append('mov rbx, %d' % (lmaopc,)) # comment so i can go back and forth between format strings to vm instructions
	asm_out.append('nop')
asm_out.append('loc_end:')
asm_out.append('leave')
asm_out.append('ret')
print('\n'.join(asm_out))