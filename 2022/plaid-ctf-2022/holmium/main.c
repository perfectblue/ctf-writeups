          ...
          //GENERATED_REWRITE_RULES_STEP_1_START//
          {
            case _LEAK1_: {
              if (1) {
                inc_cost(mem);
                u64 ret_0;
                if (get_tag(U_32(1945917207)) == U32 && get_tag(U_32(3591923955)) == U32) {
                  ret_0 = U_32(get_val(U_32(1945917207)) * get_val(U_32(3591923955)));
                  inc_cost(mem);
                } else {
                  u64 op2_1 = alloc(mem, 2);
                  link(mem, op2_1 + 0, U_32(1945917207));
                  link(mem, op2_1 + 1, U_32(3591923955));
                  ret_0 = Op2(MUL, op2_1);
                }
                u64 done = ret_0;
                link(mem, host, done);
            ...
            case _MAIN_: {
              if (1) {
                inc_cost(mem);
                u64 ctr_3 = alloc(mem, 16);
                link(mem, ctr_3 + 0, U_32(1));
                link(mem, ctr_3 + 1, U_32(0));
                link(mem, ctr_3 + 2, U_32(3));
                link(mem, ctr_3 + 3, U_32(0));
                link(mem, ctr_3 + 4, U_32(5));
                link(mem, ctr_3 + 5, U_32(0));
                link(mem, ctr_3 + 6, U_32(60));
                link(mem, ctr_3 + 7, U_32(0));
                link(mem, ctr_3 + 8, U_32(9));
                link(mem, ctr_3 + 9, U_32(0));
                link(mem, ctr_3 + 10, U_32(11));
                link(mem, ctr_3 + 11, U_32(0));
                link(mem, ctr_3 + 12, U_32(13));
                link(mem, ctr_3 + 13, U_32(0));
                link(mem, ctr_3 + 14, U_32(15));
                link(mem, ctr_3 + 15, U_32(0));
                u64 ctr_4 = alloc(mem, 16);
                link(mem, ctr_4 + 0, U_32(17));
                link(mem, ctr_4 + 1, U_32(0));
                link(mem, ctr_4 + 2, U_32(19));
                link(mem, ctr_4 + 3, U_32(0));
                link(mem, ctr_4 + 4, U_32(21));
                link(mem, ctr_4 + 5, U_32(0));
                link(mem, ctr_4 + 6, U_32(23));
                link(mem, ctr_4 + 7, U_32(0));
                link(mem, ctr_4 + 8, U_32(25));
                link(mem, ctr_4 + 9, U_32(0));
                link(mem, ctr_4 + 10, U_32(27));
                link(mem, ctr_4 + 11, U_32(0));
                link(mem, ctr_4 + 12, U_32(29));
                link(mem, ctr_4 + 13, U_32(0));
                link(mem, ctr_4 + 14, U_32(31));
                link(mem, ctr_4 + 15, U_32(0));
                u64 app_2 = alloc(mem, 2);
                link(mem, app_2 + 0, Ctr(16, 2, ctr_3));
                link(mem, app_2 + 1, Ctr(16, 2, ctr_4));
                u64 cal_5 = alloc(mem, 0);
                u64 app_1 = alloc(mem, 2);
                link(mem, app_1 + 0, App(app_2));
                link(mem, app_1 + 1, Cal(0, 0, cal_5));
                u64 cal_6 = alloc(mem, 0);
                u64 app_0 = alloc(mem, 2);
                link(mem, app_0 + 0, App(app_1));
                link(mem, app_0 + 1, Cal(0, 3, cal_6));
                u64 done = App(app_0);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 0);
                init = 1;
            ...
            case _WRITE1_: {
              if (1) {
                inc_cost(mem);
                u64 ret_1;
                if (get_tag(U_32(2515077533)) == U32 && get_tag(U_32(916807923)) == U32) {
                  ret_1 = U_32(get_val(U_32(2515077533)) * get_val(U_32(916807923)));
                  inc_cost(mem);
                } else {
                  u64 op2_2 = alloc(mem, 2);
                  link(mem, op2_2 + 0, U_32(2515077533));
                  link(mem, op2_2 + 1, U_32(916807923));
                  ret_1 = Op2(MUL, op2_2);
                }
                u64 app_0 = alloc(mem, 2);
                link(mem, app_0 + 0, ret_1);
                link(mem, app_0 + 1, U_32(3735928559));
                u64 done = App(app_0);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 0);
                init = 1;
...
int main(int argc, char* argv[]) {
  ...
  // Id-to-Name map
  const u64 id_to_name_size = 4;
  char* id_to_name_data[id_to_name_size];
  id_to_name_data[0] = "Leak1";
  id_to_name_data[1] = "Main";
  id_to_name_data[2] = "Exploit";
  id_to_name_data[3] = "Write1";
;
