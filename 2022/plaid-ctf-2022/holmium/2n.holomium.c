        ...
        case CAL: {
          ...
          //GENERATED_REWRITE_RULES_STEP_1_START//
          {
            case _SUM_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 4u)) {
                inc_cost(mem);
                ...
                u64 ret_0;
                if (get_tag(Cal(1, 1, cal_2)) == U32 && get_tag(Cal(1, 1, cal_3)) == U32) {
                  ret_0 = U_32(get_val(Cal(1, 1, cal_2)) + get_val(Cal(1, 1, cal_3)));
                  inc_cost(mem);
                } else {
                  u64 op2_1 = alloc(mem, 2);
                  link(mem, op2_1 + 0, Cal(1, 1, cal_2));
                  link(mem, op2_1 + 1, Cal(1, 1, cal_3));
                  ret_0 = Op2(ADD, op2_1);
                }
                u64 done = ret_0;
                link(mem, host, done);
            ...
            case _GEN_: {
              ...
              if (1) {
                ...
                link(mem, ctr_11 + 0, Cal(1, 2, cal_7));
                link(mem, ctr_11 + 1, Cal(1, 2, cal_10));
                u64 done = Ctr(2, 4, ctr_11);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                init = 1;
                continue;
            ...
            case _MAIN_: {
              if (1) {
                ...
                link(mem, cal_1 + 0, Cal(1, 2, cal_0));
...
int main(int argc, char* argv[]) {
  ...
  id_to_name_data[1] = "Sum";
  id_to_name_data[0] = "Main";
  id_to_name_data[3] = "Leaf";
  id_to_name_data[4] = "Node";
  id_to_name_data[2] = "Gen";
;
