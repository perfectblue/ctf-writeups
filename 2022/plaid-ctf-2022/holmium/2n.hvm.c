          ...
          //GENERATED_REWRITE_RULES_STEP_1_START//
          {
            case _GEN_: {
              ...
              if (1) {
                ...
                if (get_tag(dp0_1) == U32 && get_tag(U_32(1)) == U32) {
                  ret_5 = U_32(get_val(dp0_1) - get_val(U_32(1)));
                  inc_cost(mem);
                } else {
                  u64 op2_6 = alloc(mem, 2);
                  link(mem, op2_6 + 0, dp0_1);
                  link(mem, op2_6 + 1, U_32(1));
                  ret_5 = Op2(SUB, op2_6);
                }
                u64 cal_7 = alloc(mem, 1);
                link(mem, cal_7 + 0, ret_5);
                u64 ret_8;
                if (get_tag(dp1_2) == U32 && get_tag(U_32(1)) == U32) {
                  ret_8 = U_32(get_val(dp1_2) - get_val(U_32(1)));
                  inc_cost(mem);
                } else {
                  u64 op2_9 = alloc(mem, 2);
                  link(mem, op2_9 + 0, dp1_2);
                  link(mem, op2_9 + 1, U_32(1));
                  ret_8 = Op2(SUB, op2_9);
                }
                u64 cal_10 = alloc(mem, 1);
                link(mem, cal_10 + 0, ret_8);
                u64 ctr_11 = alloc(mem, 2);
                link(mem, ctr_11 + 0, Cal(1, 3, cal_7));
                link(mem, ctr_11 + 1, Cal(1, 3, cal_10));
                u64 done = Ctr(2, 2, ctr_11);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                init = 1;
                continue;
            ...
            case _MAIN_: {
              if (1) {
                ...
                link(mem, cal_1 + 0, Cal(1, 3, cal_0));
                u64 done = Cal(1, 0, cal_1);
                link(mem, host, done);
            ...
            case _SUM_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 1u)) {
                inc_cost(mem);
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 2u)) {
                inc_cost(mem);
                u64 cal_2 = alloc(mem, 1);
                link(mem, cal_2 + 0, ask_arg(mem, ask_arg(mem, term, 0), 0));
                u64 cal_3 = alloc(mem, 1);
                link(mem, cal_3 + 0, ask_arg(mem, ask_arg(mem, term, 0), 1));
                u64 ret_0;
                if (get_tag(Cal(1, 0, cal_2)) == U32 && get_tag(Cal(1, 0, cal_3)) == U32) {
                  ret_0 = U_32(get_val(Cal(1, 0, cal_2)) + get_val(Cal(1, 0, cal_3)));
                  inc_cost(mem);
                } else {
                  u64 op2_1 = alloc(mem, 2);
                  link(mem, op2_1 + 0, Cal(1, 0, cal_2));
                  link(mem, op2_1 + 1, Cal(1, 0, cal_3));
                  ret_0 = Op2(ADD, op2_1);
                }
                u64 done = ret_0;
                link(mem, host, done);
...
int main(int argc, char* argv[]) {
  ...
  // Id-to-Name map
  const u64 id_to_name_size = 5;
  char* id_to_name_data[id_to_name_size];
  id_to_name_data[0] = "Sum";
  id_to_name_data[1] = "Leaf";
  id_to_name_data[3] = "Gen";
  id_to_name_data[4] = "Main";
  id_to_name_data[2] = "Node";
;
