...
"""
fuckers = [0]*0x100
fuck = filter(bool, fuck.split('\n'))
for cmd in fuck:
	parts = cmd.split(';')
	if parts[0].startswith('pc* '):
		# print parts
		length, src = parts[0].split('pc* ')[1].split('>')[0].split('@')
		length = int(length)
		src = int(src.split('loc.flag+')[1])
		dst = int(parts[1].split('s ')[1].split('loc.hash+')[1])
		addend = 0
		if len(parts) > 3:
			addend = int(parts[3].split('w1')[1])
		assert length == 1
		fuckers[dst] = '[%d+%d' % (src,addend)
		# print 'copy', src, dst, addend
	elif parts[0].startswith('wx '):
		val, dst = parts[0].split('wx ')[1].split('@')
		dst = int(dst.split('loc.hash+')[1])
		# print val, dst
		length = len(val)/2
		for i in range(0, length):
			fuckers[dst+i] = val[2*i:2*i+2]
	elif parts[0].startswith('wr '):
		length, dst = parts[0].split('wr ')[1].split('@')
		length = int(length)
		dst = int(dst.split('loc.hash+')[1])
		for i in range(0, length):
			fuckers[dst+i] = 'r'
		...
		print '????', cmd
omg = open('massive.bin', 'rb').read()
out = [0]*0x30
for i, v in enumerate(fuckers):
	if v[0] == '[':
		off, add = map(int, v[1:].split('+'))
		out[off] = chr(ord(omg[i]) - add)
		print i, off, add
print ''.join(out)
