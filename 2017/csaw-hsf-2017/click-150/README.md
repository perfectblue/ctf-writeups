reading the python source, all we seem to have to do is just get a ton of points.

not really sure why supreme-400 is worth more than Lot-350 but OK.

so we just write a bot to interface with the websocket server and play for us while we do the other challenges
...
the flags are given in the achievements after completing an achievement tree.

`flag{i_hav3_2_3mit_th@t_wAs_a_B@d_1d3a}`
